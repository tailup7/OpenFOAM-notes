...
# 実行場所をこのスクリプトのあるディレクトリ、または現在ディレクトリに合わせる
cd "$(dirname "$0")"
...
# ----- constant/polyMesh/boundary の "WALL" の "type" を patch → wall に -----
...
for d in processor*/constant; do
  ln -sf ../../constant/input.data "$d/input.data"
done
...
if [ -f ./pa_convert.py ]; then
  ...
  for t in $(ls -1d [0-9]* 2>/dev/null | grep -E '^[0-9]+(\.[0-9]+)?$' | sort -V); do
    if [ -f "$t/p" ] || [ -f "$t/wallShearStress" ]; then
      ...
      python3 ./pa_convert.py --rho "${RHO}" --time "$t" --fields p wallShearStress \
        2>&1 | tee -a log.post
    fi
  done
else
